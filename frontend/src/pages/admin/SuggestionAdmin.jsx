...
import {
  ...
  Calendar, User, Lightbulb, ExternalLink, AlertCircle,
} from "lucide-react";
...
const SuggestionAdmin = () => {
  ...
  const handleReview = async (status) => {
    setSubmitting(true);
    try {
      const res = await fetch(`${API_BASE}/suggestions/${reviewing.suggestionId}/review`, {
        method: "PATCH",
  ...
  return (
    <div className="container mx-auto px-6 py-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        {[
          { label: "Total", key: "all", icon: <Lightbulb className="w-8 h-8 text-blue-600" />, border: "border-gray-200", text: "text-gray-900" },
          { label: "Pending Review", key: "pending", icon: <Eye className="w-5 h-5 text-yellow-600" />, border: "border-yellow-200", text: "text-yellow-800" },
          { label: "Approved", key: "approved", icon: <CheckCircle className="w-5 h-5 text-green-600" />, border: "border-green-200", text: "text-green-800" },
          { label: "Rejected", key: "rejected", icon: <XCircle className="w-5 h-5 text-red-600" />, border: "border-red-200", text: "text-red-800" },
        ].map(({ label, key, icon, border, text }) => (
          <div key={key} className={`bg-white border-2 ${border} rounded-lg p-4`}>
            ...
            <div>
              ...
              {(() => {
                try {
                  ...
                  return (
                    <p className="text-gray-700 bg-gray-50 p-3 rounded-lg text-sm">{suggestion.content}</p>
                  );
                }
              })()}
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6">
            ...
            <p className="text-sm text-slate-500 mb-4">
              From {reviewing.submitterName} — {reviewing.suggestionType?.replace("_", " ")}
            </p>
            {(() => {
              try {
                ...
                return (
                  <p className="text-slate-700 bg-gray-50 p-3 rounded-lg text-sm mb-4">{reviewing.content}</p>
                );
              }
            })()}
            <div className="mb-4">
